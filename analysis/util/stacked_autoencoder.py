...
from analysis.util.logistic_sgd import LogisticRegression
from analysis.util.mlp import HiddenLayer
from analysis.util.autoencoder import dA
...
class SdA(object):
    ...
    def get_output_value(self, x):
        """ 
            Computes the values of the top layer 
            In this case, the regression results (before argmax)
        """
        # self.input_x into input for first layer
        self.sigmoid_layers[0].input = x
...
def train_SdA(input_size, train_set, valid_set, test_set, finetune_lr=0.1, pretraining_epochs=15,
    ...
    """
    if hidden_layer_sizes is None and corruption_levels is None:
